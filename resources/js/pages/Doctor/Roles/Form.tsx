...
export default function DoctorRoleForm({ role, groupedPermissions }: Props) {
  const isEdit = !!role;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    isEdit ? put(`/doctor/roles/${role?.id}`) : post('/doctor/roles');
  };
  ...
  const toggleGroup = (group: string, perms: Permission[]) => {
    ...
    if (allChecked) {
      ...
      perms.forEach(perm => {
        if (!newPermissions.includes(perm.name)) newPermissions.push(perm.name);
      });
          ...
          <CardContent className="pt-5">
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="space-y-4">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Select permissions to grant to this role</p>
                  </div>
                    ...
                    return (
                      <div key={group} className="bg-muted/20 p-4 rounded-lg border">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          />
                          <label htmlFor={`group-${group}`} className="text-sm font-medium uppercase tracking-wider text-muted-foreground cursor-pointer">
                            {group}
                          </label>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 pl-7">
                          {perms.map((perm) => (
                            <div key={perm.id} className="flex items-center gap-3">
                              ...
                              />
                              <label htmlFor={`perm-${perm.id}`} className="text-sm cursor-pointer">{perm.name}</label>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-2">
                <Link href="/doctor/roles" className="w-full sm:w-auto">
