...
interface Props {
  user?: User;
  roles: Role[]; // solo los roles de la clínica del doctor
  currentRoles?: string[];
}

export default function DoctorUserForm({ user, roles, currentRoles }: Props) {
  const isEdit = !!user;
...
const { data, setData, post, put, processing, errors } = useForm<FormData>({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isEdit) {
      put(`/doctor/users/${user?.id}`, data);
    } else {
      post('/doctor/users', data);
    }
  };

  const breadcrumbs: BreadcrumbItem[] = [
    { title: 'User Management', href: '/doctor/users' },
        ...
        <Card className="max-w-3xl mx-auto">
          <CardHeader className="pb-3">
            ...
            <p className="text-sm text-muted-foreground">
              {isEdit ? 'Update user data and roles' : 'Enter user data and assign roles'}
            </p>
          </CardHeader>
          ...
          <CardContent className="pt-5">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-2">
                <Link href="/doctor/users" className="w-full sm:w-auto">
